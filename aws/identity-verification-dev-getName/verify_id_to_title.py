...
def analyze_ID(image):
    ...
    j = call_textract_analyzeid(document_pages=["s3://asce4s-test-id-bucket/"+image],
                                boto3_textract_client=textract_client)

    first_name = j['IdentityDocuments'][0]['IdentityDocumentFields'][0]['ValueDetection']['Text']
    last_name = j['IdentityDocuments'][0]['IdentityDocumentFields'][1]['ValueDetection']['Text']

    street = j['IdentityDocuments'][0]['IdentityDocumentFields'][17]['ValueDetection']['Text']
    _zip = j['IdentityDocuments'][0]['IdentityDocumentFields'][5]['ValueDetection']['Text']
    city = j['IdentityDocuments'][0]['IdentityDocumentFields'][4]['ValueDetection']['Text']
    state = j['IdentityDocuments'][0]['IdentityDocumentFields'][6]['ValueDetection']['Text']
...
def handler(event, context):
    ...
    payload, applicant_name = analyze_ID(body["image"])
    res={}
